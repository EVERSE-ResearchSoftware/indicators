...
def load_local_schema(schema_path):
    ...
    try:
        ...
        pytest.fail(f"Failed to decode JSON from schema file {schema_path}: {e}", pytrace=False)
    except Exception as e:
        pytest.fail(f"An unexpected error occurred while loading schema {schema_path}: {e}", pytrace=False)
...
def validate_json_files_using_schema(schema_file_path, json_file_path):
    ...
    assert not validation_errors, "The schema validation failed for one or more files:\n" + "\n".join(validation_errors)
